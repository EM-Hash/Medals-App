...
const Country = ( {country, changeMedalNum, handleDelete} ) => {
    ...
    let medalButtons = country.medals.map((medal) => {
        ...
        return (
            <Grid item xs={4} key={medal.name + "Medal"} mt={2}>
                ...
                <Button key={medal.name + "ButtonPlus"} className={"add" + upperMedalName + "MedalButton"} 
                onClick={() => {changeMedalNum(country.name, medal.name, 1)}} color="info" variant="contained" sx={{margin: 1}}>
                    <FontAwesomeIcon icon={faPlus} />
                </Button>
                <Button key={medal.name + "ButtonMinus"} className={"add" + upperMedalName + "MedalButton"} 
    ...
    let totalMedal = {
        ...
        'count': country.medals.reduce((total, medal) => {return total + medal.count}, 0)
    };
